// kulms-shim.js — chrome.* API shim for Android WebView
// This script is injected BEFORE any extension content scripts.
// It creates a fake `chrome` namespace that bridges to Kotlin via
// @JavascriptInterface (Android.kulmsStorageRequest).
...
(function () {
  ...
  // ---- callback registry for async Kotlin responses ----
  var _cbId = 0;
  var _callbacks = {};
  ...
  function postToKotlin(action, payload, callback) {
    var id = String(++_cbId);
    if (callback) _callbacks[id] = callback;
    var msg = Object.assign({ action: action, callbackId: id }, payload);
    try {
      Android.kulmsStorageRequest(JSON.stringify(msg));
    } catch (e) {
  ...
  window.__kulmsNativeFetch = function (url) {
    return new Promise(function (resolve, reject) {
      postToKotlin("nativeFetch", { url: url }, function (result) {
  ...
  var storageLocal = {
    get: function (keys, callback) {
      ...
      postToKotlin("get", { keys: keyList }, function (result) {
    ...
    set: function (items, callback) {
      postToKotlin("set", { items: items }, function () {
    ...
    remove: function (keys, callback) {
      ...
      postToKotlin("remove", { keys: keyList }, function () {
        if (callback) callback();
      });
    },
    clear: function (callback) {
      postToKotlin("clear", {}, function () {
        if (callback) callback();
      });
    }
  };
  ...
  // Shim-internal message store. settings.js resets window.__kulmsOverrideMessages
  // to null on load (var hoisting), so we keep an independent copy that survives.
  // ContentScriptInjector calls __kulmsSetShimMessages() after locale data injection.
  var _shimMessages = null;

  window.__kulmsSetShimMessages = function (data) {
    _shimMessages = data;
  };

  function _resolveMessage(source, key, substitutions) {
  ...
  var i18n = {
    getMessage: function (key, substitutions) {
      // 1. Check window.__kulmsOverrideMessages (set by settings.js after async load)
      var result = _resolveMessage(window.__kulmsOverrideMessages, key, substitutions);
      if (result) return result;

      // 2. Fall back to shim-internal messages (pre-loaded, always available)
      result = _resolveMessage(_shimMessages, key, substitutions);
      if (result) return result;
  ...
  console.log("[kulms-shim] chrome.* API shim installed (Android)");
})();
